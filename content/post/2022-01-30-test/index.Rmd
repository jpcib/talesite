---
title: Sondeo Medios - test
author: Jcib
date: '2022-01-30'
slug: []
categories:
  - R
tags:
  - test
  - sondeo
---

Tabla de contenidos de medios argentinos mediante RSS.
---

Si no aparece, el **formato de fecha** desde-hasta es: **2021-11-20 ... 2022-02-07**

```{r lib, include=FALSE}
library(flexdashboard)
library(readr)
library(dplyr)
library(tidyr)
library(magrittr)
library(DT)
```

```{r tabla, include=FALSE}
tabla7 <- read_csv("pre_tabla_02-24-A.csv")
tabla6 <- read_csv("pre_tabla_02-22-B.csv")
tabla5 <- read_csv("pre_tabla_02-21-B.csv")
tabla4 <- read_csv("pre_tabla_02-21-A.csv")
tabla3 <- read_csv("pre_tabla_02-18-B.csv")
tabla2 <- read_csv("pre_tabla_02-18-A.csv")
tabla1 <- read_csv("pre_tabla_02-17.csv")
tabla <- bind_rows(tabla7,tabla6,tabla5,tabla4,tabla3,tabla2,tabla1) %>% 
  distinct(titulo, .keep_all = TRUE) %>% 
  print()
tabla$medio <- as.factor(tabla$medio)
```

## Noticias:

Filtrar según medio, fecha, o palabras clave. El buscador de arriba a la derecha permite buscar en todas las columnas.

```{r dt plot, echo=FALSE, message=FALSE, warning=FALSE}

DT::datatable(tabla, filter = 'top', 
              escape = FALSE, 
              class = 'cell-border stripe',
              options = list(pageLength = 50, autoWidth = TRUE,
                             columnDefs = list(list(width = '200px', targets = 3)),
                             initComplete = JS(
              "function(settings, json) {",
              "$(this.api().table().header()).css({'background-color': '#000', 'color': '#fff'});",
              "}")),
               rownames = FALSE
              )

# datatable(head(iris, 20), options = list(
#   initComplete = JS(
#     "function(settings, json) {",
#     "$(this.api().table().header()).css({'background-color': '#000', 'color': '#fff'});",
#     "}")
# ))
```
